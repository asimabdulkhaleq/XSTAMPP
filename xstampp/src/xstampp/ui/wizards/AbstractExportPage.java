...
public abstract class AbstractExportPage extends AbstractWizardPage implements IExportPage {
	...
	/**
	 * a constant which is used by a {@link JAXBExportJob} sets it as a parameter in
	 * the creation of the pdf to print in landscape mode
	 */
	public static final String A4_LANDSCAPE = "A4Landscape"; //$NON-NLS-1$

	/**
	 * a constant which is used by a {@link JAXBExportJob} sets it as a parameter in
	 * the creation of the pdf to print in portrait mode
	 */
	 ...
	 * @param attachment
	 *            the {@link FormAttachment} for the {@link FormLayout}'s top
	 *            constraint
	...
	/**
	 * calls
	 * {@link AbstractExportPage#addPathComposite(FormAttachment, String[], Composite, int)}
	 * with {@link PathComposite#PATH_DIALOG} as style argument.
	 */
	protected Composite addPathComposite(FormAttachment topAttachment, String[] filters, Composite control) {
		return addPathComposite(topAttachment, filters, control, PathComposite.PATH_DIALOG);
	}
	 ...
	 *            {"*.haz","*.hazx"}
	 * @param control
	 *            the parent control in which the Path composite is integrated.
	 * @param style
	 *            the style, must be one of the constants defined in PathComposite
	 * 
	 * @return the path composite, which can be used as reference for the
	 *         {@link FormLayout}
	 */
	protected Composite addPathComposite(FormAttachment topAttachment, String[] filters, Composite control, int style) {
		this.pathChooser = new PathComposite(filters, control, style);
		FormData data = new FormData();
		data.top = topAttachment;
		this.pathChooser.setLayoutData(data);
		return this.pathChooser;
	}
